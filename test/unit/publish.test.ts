...
import { HACKAGE_CANDIDATES_URL, postReleaseCandidate, publishRCDocumentation } from "../../src/publish";

const filename = "my-package-1.0.0.tar.gz";
const docsFilename = "my-package-1.0.0-docs.tar.gz";
const docsUrl = "https://hackage.haskell.org/package/my-package-1.0.0/candidate/docs";
...
describe("publishRCDocumentation", () => {
  it("returns the status code when request is successful", async () => {
    ...
    const statusCode = await publishRCDocumentation(docsSdistPath, docsUrl, hackageToken);
  ...
  it("throws an error on unsuccessful request", async () => {
    const errorMsg = "Error message from server";
    const axiosPutStub = sinon.stub(axios, "put").rejects({ message: errorMsg });

    const request = publishRCDocumentation(docsSdistPath, docsUrl, hackageToken);
